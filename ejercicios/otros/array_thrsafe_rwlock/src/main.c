...
#include "array_thrsafe_rwlock.h"

void print_array(const char* name, array_rwlock_t* array);
void test_arrays(array_rwlock_t* array1, array_rwlock_t* array2);
void* test_array(void* data);

int main() {
    array_rwlock_t* array1 = array_create(100);
    array_rwlock_t* array2 = array_create(100);
...
void test_arrays(array_rwlock_t* array1, array_rwlock_t* array2) {
	...
	for ( size_t current = 0; current < thread_count; ++current )
		pthread_create( threads + current, NULL, test_array, current % 2 ? array2 :
			array1 );
...
void* test_array(void* data) {
    array_rwlock_t* array = (array_rwlock_t*)data;

	for ( size_t index = 0, count = 1000 + rand() % 10000; index < count;
		++index ) {
		size_t num = rand() % 100;
		if ( num >= 10 )
