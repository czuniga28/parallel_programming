...
int ProducerTest::run() {
  ...
  for ( size_t index = 0; index < this->packageCount; ++index ) {
    this->produce(this->createMessage(index)); // this is a real message
  }

  // Produce an empty message to communicate we finished
  // this message will be tell everyones to stop
  this->produce(NetworkMessage()); //here we send a stop siganl 
