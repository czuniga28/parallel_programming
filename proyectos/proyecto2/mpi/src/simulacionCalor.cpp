...
int recibirEstados(std::vector<infoSimulacion>& infoSimulaciones) {
  int inicio = 0, fin = infoSimulaciones.size();
  ...
  // Índices de inicio y fin para repartir las simulaciones
  int inicio_index = -1, fin_index = -1;

  // Procesos MPI
  for (int source = 1; source < process_count; ++source) {
    // Calcular los índices de inicio y fin para las simulaciones
    inicio_index = calcular_inicio(source, fin, process_count, inicio);
    fin_index = calcular_fin(source, fin, process_count, inicio);

    for (int simulacion_index = inicio_index; simulacion_index < fin_index;
    ++simulacion_index) {
      // Recibir el estado de simulación del proceso
      if (MPI_Recv(&infoSimulaciones[simulacion_index].estado, 1, MPI_INT64_T,
        source, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE) != MPI_SUCCESS) {
        std::cerr << "Error: No se pudo recibir la cantidad de estados"
        << std::endl;
        error = EXIT_FAILURE;
      }
    }
...
int calcular_inicio(int rank, int end, int workers, int begin) {
  // Calcular la longitud total para repartir
  const int range = end - begin;
  // Devolver el inicio para el proceso actual
  return begin + rank * (range / workers) + std::min(rank, range % workers);
}

int calcular_fin(int rank, int end, int workers, int begin) {
  //  Se usa calcular_inicio para obtener el siguiente índice
  return calcular_inicio(rank + 1, end, workers, begin);
}
