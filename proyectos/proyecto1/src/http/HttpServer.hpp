...
// maximo de conexiones se define con los nucleos
#define MAX_CONNECTIONS sysconf(_SC_NPROCESSORS_ONLN)
...
class HttpServer : public TcpServer {
 ...
 protected:
  ...
  /// @brief Donde se almacenaran las webApp y seran consultadas por
  // consumidores
  std::vector<HttpApp*> applications;
  /// @brief Consumidores del programa
  std::vector<HttpConnectionHandler*> connectionHandlers;
  /// cantidad de consumidores
  size_t workersCount = MAX_CONNECTIONS;
  /// @brief Cola de la que consumen connectionshandlers
  Queue<Socket>* socketsQueue;  // esto era consumersQueue
 ...
 public:
  ...
  /// @brief brinda acceso global a la unica instancia existente en el programa
  /// de HttpServer
  /// @return referencia a la unica instancia HttpServer(por ser static)
  static HttpServer& getInstance();

  inline size_t getWorkers() {
 ...
 protected:
  ...
  /// Hace que los consumidores (Connection Handlers) reciban un socket
  /// vacio, sennal de finalizacion
  void sendEmpySockects();

  /// @brief Espera la finalizacion haciendo uso de la herencia de Thread
  void waitingFinish();


  // TODO(estudiante) Cuando connectionhandler este lista, borrar de aca
  // para abajo
