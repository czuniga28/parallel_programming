...
int controller_run(size_t thread_count) {
  ...
  for (size_t fileNumber = 0; fileNumber < array.count; fileNumber++) {
    ...
    shared_data->password = NULL;  // Inicializa a NULL al inicio de cada iteración
    ...
    if (shared_data->password != NULL) {
        ...
        array.dynamicArrayInner[fileNumber].password = (char*)calloc(size + 1, sizeof(char));
        // Copy the password to the dynamic array
        strcpy(array.dynamicArrayInner[fileNumber].password, shared_data->password);
        // Free the memory allocated for the password in shared_data
        free(shared_data->password);
        shared_data->password = NULL;  // Establece a NULL después de liberar la memoria
    }

    // Free the memory allocated for the file path
    free(shared_data->path);
  }
