...
typedef struct {
  ...
  sem_t can_produce;  // semaforo productor
  sem_t can_consume;  // semaforo consumidor
} shared_data_t;
...
int main(int argc, char* argv[]) {
  ...
  if (shared_data) {
    error = analyze_arguments(argc, argv, shared_data);  // analisis de args
    if (error == EXIT_SUCCESS) {
      ...
      if (shared_data->buffer) {  
        // se inicia el semaforo del productor con la capacidad del buffer
        sem_init(&shared_data->can_produce, /*pshared*/ 0,
          shared_data->buffer_capacity);

        // se inicia el semaforo del consumidor en 0
        ...
        error = create_threads(shared_data);  // creamos los hilos
        ...
        //Limpieza de semaforos en orden inverso a como se crearon
...
void* produce(void* data) {
  ...
  for (size_t round = 0; round < shared_data->rounds; ++round) {
    for (size_t index = 0; index < shared_data->buffer_capacity; ++index) {
      ...
      // signal(can_consume)
      sem_post(&shared_data->can_consume);  // avisa que hay algo para consumir
    }
  }
...
void* consume(void* data) {
  ...
  for (size_t round = 0; round < shared_data->rounds; ++round) {
    for (size_t index = 0; index < shared_data->buffer_capacity; ++index) {
      ...
      // signal(can_produce)
      sem_post(&shared_data->can_produce);  // avisa que se puede producir
    }
  }
