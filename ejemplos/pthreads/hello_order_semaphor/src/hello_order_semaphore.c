...
int main(int argc, char* argv[]) {
  ...
  if (shared_data) {
    ...
    free(shared_data->can_greet);  // libera la memoria solicitada de la cantidad de hilos
  } else {
...
int create_threads(shared_data_t* shared_data) {
  ...
  if (threads && private_data) {
    ...
    thread_number++) {
      // can_greet[thread_number] := create_semaphore(not thread number)
...
void* race(void* data) {
  ...
  printf("Hello from secondary thread %" PRIu64 "of %" PRId64 "\n", 
  private_data->thread_number, shared_data->thread_count);

  // deja al siguiente hilo realizar la tarea
